    ...
    T: Clone + Serialize + for<'de> Deserialize<'de> + Send + Sync + 'static,
{
    ...
    pub async fn bulk_create_records(&self, items: Vec<T>) -> Result<Vec<Option<T>>> {
        ...
        for (i, item) in items.iter().enumerate() {
            let item_json = serde_json::to_string(item)
                .context("Failed to serialize item for bulk insert")?;
        ...
        self.db.conn().query(transaction).await.context("Failed to execute bulk insert transaction")?;
    ...
    pub async fn run_custom_query<P: Serialize>(
        ...
        query
            .execute()
            .await
            .context(format!("Failed to execute custom query on {}", self.table_name))
    }
    ...
    pub async fn record_exists(&self, record_id: &str) -> Result<bool> {
        ...
        let result: Vec<serde_json::Value> = self.db
            ...
            .execute()
            .await
            .context(format!("Failed to check if record exists in {}", self.table_name))?;
