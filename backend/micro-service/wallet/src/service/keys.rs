...
impl WalletService {
    ...
    pub async fn store_wallet_key(
        ...
        if let Some(wallet_key_db) = &self.wallet_key_db {
            ...
            match wallet_key_db.create_record(wallet_key.clone()).await {
                Ok(Some(_)) => {
                    ...
                    if let Some(wallet_db) = &self.wallet_db {
                        ...
                        wallet_db
                            .update_record(wallet_id, updated_wallet)
                            .await
                            .map_err(|e| {
                ...
                Ok(None) => {
                    ...
                    Err(AppError::DatabaseError(anyhow::anyhow!(
                        "Failed to store wallet key"
                    )))
                }
                Err(e) => {
                    error!("Failed to store wallet key in database: {}", e);
                    Err(AppError::DatabaseError(anyhow::anyhow!(format!(
                        "Failed to store wallet key: {}",
                        e
                    ))))
                }
            ...
            Err(AppError::ServerError(anyhow::anyhow!(
                "Wallet key database not available"
            )))
        }
    }
    ...
    pub async fn get_wallet_key_by_wallet_id(&self, wallet_id: &str) -> AppResult<WalletKey> {
        if let Some(wallet_db) = &self.wallet_db {
            ...
            let wallet = wallet_db
                ...
                .map_err(|e| {
                    error!(
                        "Database error when fetching wallet for key retrieval: {}",
                        e
                    );
                    AppError::DatabaseError(anyhow::anyhow!(e))
                })?
                ...
                let key = wallet_key_db
                    ...
                    .map_err(|e| {
                        ...
                        AppError::DatabaseError(anyhow::anyhow!(format!(
                            "Failed to fetch wallet key: {}",
                            e
                        )))
                    })?
                    .ok_or_else(|| {
                        AppError::NotFoundError(format!(
                            "Wallet key with ID '{}' not found",
                            key_id
                        ))
                    })?;
    ...
    pub async fn get_wallet_encrypted_data(
        &self,
        wallet_id: &str,
    ...
    pub async fn rotate_master_key(
        ...
        self.update_wallet_key(wallet_id, &new_encrypted_data)
            .await?;
    ...
    pub async fn rotate_all_master_keys(
        ...
        if let Some(wallet_key_db) = &self.wallet_key_db {
            ...
            let old_keys = wallet_key_db
                ...
                .map_err(|e| {
                    ...
                    AppError::DatabaseError(anyhow::anyhow!(format!(
                        "Failed to fetch keys for rotation: {}",
                        e
                    )))
                })?;
            ...
            for key in old_keys {
                ...
                match pin_provider(wallet_id) {
                    Ok(pin) => {
                        ...
                        match self
                            .rotate_master_key(wallet_id, &pin, new_encryption_service)
                            .await
                        {
                            Ok(_) => {
                                successful += 1;
        ...
        info!(
            "Master key rotation completed: {} successful, {} failed",
            successful,
            failed_wallets.len()
        );
